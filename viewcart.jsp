...
.product .product-img .products-label {
  position: absolute;
  right: 65px;
    top: 15px;
 
}
.product .product-img .products-label>span {
  border: 2px solid;
  padding: 2px 10px;
  font-size: 12px;
}

.product .product-img .products-label>span.sale {
  background-color: #FFF;
  border-color: #D10024;
  color: #D10024;
}

.product .product-img .products-label>span.new {
...
{  
var ssid=id;
	var url="addqty?id="+ssid +"&quantity="+x;
...
if(val.indexOf("Maximum quantity")>-1){
	alert(val);
}
}  
} 
...
$(window).load(function(){  
	...
	var val=request14.responseText;  
		...
		document.getElementById('cartin1').innerHTML=val;
	}}
	var request15;  
	function cartarea()  
	{  
	...
	var val=request16.responseText;  
		...
		document.getElementById('t1').innerHTML=val;
	}}
	$(window).load(function(){  
		   this.wishint(); 
		    }); 
...
<nav id="navigation">
	...
	<%
					...
					int i=0;
				        ...
				        PreparedStatement ps1=con.getPreparedstatement("select loid from Login where loemail =?");	 
						 ...
						 PreparedStatement ps2=con.getPreparedstatement("select count(cid) from carttable where cuseremail=?");	 
						 ps2.setInt(1, loid);
	...
	int q=0;
			...
			for(int o=0;o<m;o++){
				PreparedStatement ps2=con.getPreparedstatement("select producttable.proname,producttable.proprice,producttable.proid, producttable.discountprice,producttable.procategory,producttable.prooffr,carttable.cproqut  from carttable \n" + 
						"inner join producttable on carttable.cproid=producttable.proid  \n" + 
						"inner join Login on carttable.cuseremail=  Login.loid \n" + 
						"WHERE Login.loid=?");	
				ps2.setInt(1, loid);
	       ...
	       p++;
	        ...
	        %><div class="product">
									...
									<div class="input-number">
										...
										<input type="number" id="quty1" name = "qty" value =<%=proqut %> >
									...
									<div class="products-label">
											 <span class="new" onclick="add(<%=id%>)">Update Qty</span>	
											</div>	</div>
										</form>	
								       ...
								       <br>  <br>
											...
											<div class="add-to-cart">
												<button class="add-to-cart-btn"  onclick = "deletecartitem(<%=id%>)"><i class="fa fa-shopping-cart"></i>Delete  Item</button>
											</div>
										</div>
										<!-- /product -->
...
<span id="cartin1"></span> Item(s) selected<br>
<h5>    SUBTOTAL:  <span id="t1"></span></h5>

<div class="cart-btns">
<div class="add-to-cart">
<button class="add-to-cart-btn" onclick="window.location.href='checkout.jsp'">   Checkout<i class="fa fa-arrow-circle-right"></i></button>
</div>	
		</div>					
</div></div></div>
		<!-- NEWSLETTER -->
